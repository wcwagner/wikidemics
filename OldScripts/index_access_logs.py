...
def process_pagecounts_file(file_dest, output_dir):
    ...
    with sqlite3.connect(db_file) as connection:
        # how many disk pages SQLite will hold in meory at once per open db file
        # connection.execute('PRAGMA cache_size=2097152;')
        connection.text_factory = str
        with open("/media/william/Passport/wikipedia/index/schema.sql") as schema:
        ...
        with gzip.open(file_dest, "rb") as pagecounts:
            ...
            for line in pagecounts:
                ...
                if len(rows) == 10000:
                    connection.executemany("INSERT INTO page_view_stats "\
                                           "(project, title, accesses, size) "\
                                           "VALUES (?, ?, ?, ?);", rows)
                    # reset rows to be inserted
                    rows = list()
...
if __name__ == "__main__":
    ...
    if args:
        ...
        output_dir = "/media/william/Passport/wikipedia/index/{year}/{month}/{day}"
